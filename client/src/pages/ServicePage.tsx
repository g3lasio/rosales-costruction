...
export default function ServicePage() {
  ...
  return <><Seo title={service.name[locale]} description={service.summary[locale]} path={`/services/${service.slug}`} image={service.image} schema={{ "@context": "https://schema.org", "@type": "Service", name: service.name[locale], description: service.summary[locale], provider: { "@type": "HomeAndConstructionBusiness", name: "Rosales Landscaping & Construction Inc" }, areaServed: ["Napa", "Vallejo", "North Bay"] }} />
    <section className="service-hero"><div className="service-hero-image"><img src={service.image} alt={service.alt[locale]} fetchPriority="high" decoding="async" /></div><div className="container service-hero-content"><Link href="/services/" className="back-link"><ArrowLeft size={16} /> {t.back}</Link><span className="eyebrow">{service.eyebrow[locale]}</span><h1>{service.title[locale]}</h1><p>{service.summary[locale]}</p><a className="button button-primary" href="#estimate">{t.estimate}<ArrowRight size={16} /></a></div></section>
